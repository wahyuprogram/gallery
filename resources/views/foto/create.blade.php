...
<div class="card" style="max-width: 600px; margin: 0 auto; border-top: 4px solid #7E57C2;">
    ...
    <form action="{{ route('foto.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="JudulFoto" id="JudulFoto" value="{{ old('JudulFoto') }}" required>
            @error('JudulFoto') <div class="error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label for="DeskripsiFoto">Deskripsi Foto</label>
            <textarea name="DeskripsiFoto" id="DeskripsiFoto" required>{{ old('DeskripsiFoto') }}</textarea>
        ...
        <div class="form-group">
            <label for="LokasiFile">Foto (JPG, PNG, WEBP)</label>
            <input type="file" name="LokasiFile" id="LokasiFile" accept="image/*" required>
